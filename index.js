...
function processDuplicateFree(list, callback) {
    //this is definitely not the best way to do this I'm sure.
    for(var i = 0; i < list.length - 1; i++){
        if(list[i] === list[i+1]) list.splice(i+1, 1);
    }
    if(list[list.length - 2] === list[list.length - 1]) list.splice(list.length - 1);
    return callback(list);
}

//I have no idea what these comments below are supposed to tell me.....
...
function stringSmash(strings) {
    //great reference for this topic: https://www.youtube.com/watch?v=g1C40tDP0Bk
    //Ryan Hamblin did a very poor job explaining this method. And half of his example included the map function which isn't allowed here. And
    //he couldn't explain why this is better or cleaner than a for loop.
    const sum = strings.reduce((accum, currVal) =>{
...
function getFullNames(runners) {
    ...
    runners.forEach((runner) => {
        out.push(`${runner.last_name}, ${runner.first_name}`);
    });
    return out;
}
...
function getRunnersByTShirtSize(runners, shirtSize) {
    return runners.filter((runner) =>{
        if(runner.shirt_size === shirtSize) return runner;
    });
}
 ...
 * Counter 1 uses a helper method called conterMaker, counter2 just iterates a variable count when called
 *
